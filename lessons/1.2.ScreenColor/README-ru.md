...
Давайте пристальнее взглянем на `index.lua`:

* `main` является так называемым пространством имён, содержащим экземпляр приложения `application`
    * `Lua` не имеет понятия пространства имён, однако, его легко симулировать с помощью таблиц
* `application` является экземпляром приложения
    * содержит подсистемы вроде ресурсов, узлов сцен и т.д.
* `camera` является основной камерой, используемой приложением
    * является "окном" в игровую сцену
* `clearColor` является свойством камеры для выставления так называемого `очищающего цвета` (`clearing color`)
    * принимает цвет в формате RGB
* `{0, 1, 0}` является представлением зелёного цвета в RGB
    * представлен в `Lua` массивом

Более подробно на API `ogstudio` мы остановимся позже. Теперь же
давайте выставим красный цвет вместо зелёного:
